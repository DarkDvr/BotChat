...
from config import (
    ...
    BOTRAM_LOOP_PROMPT, BOTRAM_MEMORY_RECALL_PROMPT
)
...
class BotRAM:
    ...
    def gatherMemoryContext(self, botName, targetName):
        ...
        if targetName != "-ambient-":
            ...
            """, (f'%{targetName}%', f'%{botName}%', BOTRAM_PAST_CONVOS_LIMIT)).fetchall()
        ...
        ambientConvos = self.conn.execute("""
            ...
            ORDER BY c.last_valid_timestamp DESC LIMIT ?
        """, (botName, BOTRAM_AMBIENT_CONVOS_LIMIT)).fetchall()
    ...
    def processIncoming(self, payload):
        ...
        # Write conversation state to payload
